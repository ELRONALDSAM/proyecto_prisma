...
function initListeners() {
  el('img-file')?.addEventListener('change', async e => { const f = e.target.files[0]; if (!f) return; if (f.size > 5242880) return showToast('Imagen > 5MB', 'err'); const fd = new FormData(); fd.append('image', f); const token = localStorage.getItem('token'); try { showToast('Subiendo imagen...'); const res = await fetch(`${CONFIG.apiBase}/products/upload`, { method: 'POST', headers: { 'Authorization': 'Bearer ' + token }, body: fd }); if (!res.ok) { const errData = await res.json(); throw new Error(errData.error || 'Error al subir imagen'); } const data = await res.json(); el('img-preview').src = data.url; el('img-preview-wrap').classList.add('has-img'); el('field-img').value = data.url; showToast('Imagen subida con éxito ✓'); } catch (err) { showToast(err.message || 'Error al subir imagen', 'err'); console.error(err); } });
